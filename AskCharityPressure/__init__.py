...
class C(BaseConstants):
    ...
    link1 = "https://www.dropbox.com/scl/fi/tn2jsi4n2r0f5t4qmz199/IMG_3670.MOV?dl=0&rlkey=yrvxqegwcq7idogl6ovnqq7cd"
...
class Player(BasePlayer):
    ...
    q_video = models.IntegerField(
        label="3. Please state whether the following is True or False: 'The video shows a staff member at Save the Children asking for donations.' ",
    ...
    donationqother = models.IntegerField(
        label="2. Are you already a donor for any other charity?",
        choices=[[1, "Yes"], [0, "No"]],
    )
    charityq = models.IntegerField(
        label="3. Do you think Save the Children is a charity worth donating to?",
        choices=[1, 2, 3, 4, 5],
        widget=widgets.RadioSelectHorizontal,
    )
    temptationqA = models.IntegerField(
        label="4. How tempting did the video made Option A (me:£5, charity:£0) seem?",
        blank=True,
    )
    temptationqB = models.IntegerField(
        label="5. How tempting did the video made Option B (me:£1, charity:£8) seem?",
        blank=True,
    )
    emotions_ant = models.IntegerField(
        label="1. To what extent did you anticipate these emotions before watching the video?",
        blank=True,
    )
    ...
    qb = models.IntegerField(
        label="",
        choices=[
            [1, "Option A (me:£5, charity:£0)"],
            [0, "Option B (me:£1, charity:£8)"],
        ],
        blank=False,
    )

    ##Attention questions video
    controlq_fundraising = make_field3(
        label="...a woman called Gemma who introduced herself as the Head of Fundraising at Save the Children."
    )
    controlq_flute = make_field3(label="...a gas mask.")
    controlq_airplane = models.IntegerField(
        choices=[[1, "False"], [0, "True"]],
        label="...a girl on an airplane.",
        widget=widgets.RadioSelect,
    )

    ##Empathy questionaire
    qemp1 = make_field2agree(
        label="I can easily tell if someone else wants to enter a conversation."
    )
    qemp3 = make_field2agree(label="I really enjoy caring for other people.")
    qemp4 = make_field2disagree(
        label="I find it hard to know what to do in a social situation."
    )
    qemp8 = make_field2disagree(
        label="I often find it difficult to judge if something is rude or polite."
    )
    ...
    qemp11 = make_field2agree(
        label="I can pick up quickly if someone says one thing but means another."
    )
    qemp12 = make_field2disagree(
        label="It is hard for me to see why some things upset people so much."
    )
    qemp13 = make_field2agree(
        label="I find it easy to put myself in somebody else’s shoes."
    )
    qemp14 = make_field2agree(label="I am good at predicting how someone will feel.")
    qemp15 = make_field2agree(
        label="I am quick to spot when someone in a group is feeling awkward or uncomfortable."
    )
    qemp18 = make_field2disagree(
        label="I can’t always see why someone should have felt offended by a remark."
    )
    ...
    qemp26 = make_field2agree(
        label="I can easily tell if someone else is interested or bored with what I am saying."
    )
    qemp28 = make_field2agree(
        label="Friends usually talk to me about their problems as they say that I am very understanding."
    )
    qemp29 = make_field2agree(
        label="I can sense if I am intruding, even if the other person doesn’t tell me."
    )
    qemp31 = make_field2disagree(
        label="Other people often say that I am insensitive, though I don’t always see why."
    )
    qemp34 = make_field2agree(
        label="I can tune into how someone else feels rapidly and intuitively."
    )
    qemp35 = make_field2agree(
        label="I can easily work out what another person might want to talk about."
    )
    qemp36 = make_field2agree(
        label="I can tell if someone is masking their true emotion."
    )
    qemp38 = make_field2agree(label="I am good at predicting what someone will do.")
    qemp39 = make_field2agree(
        label="I tend to get emotionally involved with a friend’s problems."
    )

    # Feedback questions
    q_feedback = models.LongStringField(
    ...
    q_feedback_pilot = models.LongStringField(
        label="If you found any instructions unclear or confusing, please let us know here.",
        blank=True,
    )

    def set_error_message(player, value):
        correct_answers = {"q1A": 1, "q1B": 1, "q_video": 1}
...
class Attention1(Page):
    form_model = "player"
    form_fields = ["controlq_fundraising", "controlq_flute", "controlq_children"]

    @staticmethod
    def before_next_page(player, timeout_happened):
        player.sum_correct = (
            player.controlq_fundraising
            + player.controlq_flute
            + player.controlq_airplane
        )
...
class FailedAttention(Page):
    form_model = "player"
    form_fields = ["controlq_fundraising", "controlq_flute", "controlq_airplane"]
